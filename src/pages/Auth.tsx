        ...
        <div className="flex bg-secondary rounded-full p-1 mb-8">
          {(["signup", "login"] as const).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
              className={`flex-1 py-2.5 text-sm font-semibold rounded-full transition-all duration-200 font-body capitalize ${
                tab === t
        ...
        <div className="bg-card border border-border rounded-2xl p-8 space-y-4">
            ...
            {tab === "signup" && (
              ...
              >
                <div>
                  ...
                  />
                  {errors.name && <p className="text-[11px] text-destructive mt-1 font-body">{errors.name}</p>}
                </div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-border bg-secondary px-4 py-3 text-sm font-body text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-all"
            />
            {errors.email && <p className="text-[11px] text-destructive mt-1 font-body">{errors.email}</p>}
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-border bg-secondary px-4 py-3 text-sm font-body text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition-all"
            />
            {errors.password && <p className="text-[11px] text-destructive mt-1 font-body">{errors.password}</p>}
          </div>
          ...
          <motion.button
            ...
            className="w-full mt-2 rounded-full bg-gold py-3.5 text-sm font-semibold text-primary-foreground hover:bg-gold-hover transition-colors animate-gold-pulse"
          >
            {tab === "signup" ? "Create Account & Claim 1,000 Coins" : "Log In"}
          </motion.button>
